function [TF, ts] = sprint_stft(F,stft_params)
% SPRiNT_stft: Compute a locally averaged short-time Fourier transform (for
% use in SPRiNT)
% 
% Segments of this function were adapted from the Brainstorm software
% package:s
...
sfreq = stft_params.samp_rate;                
WinLength = stft_params.window_length;   
WinOverlap = stft_params.window_overlap;
avgWin = stft_params.n_windows_avg;
nTime = size(F,2);
